...
var getTestsList = ()=>{
	var TestTask = [ "neformat", "null", "[]", "{}", '{"equality":0,"compare":0}' ];
	if(1){
		TestTask.push('{"formula":0}');
		TestTask.push('{"formula":"mef"}');
		TestTask.push('{"formula":"mef","from":[]}');
		TestTask.push('{"formula":"mef","from":{}}');
		TestTask.push('{"formula":"mef","from":{"a":null}}');
		TestTask.push('{"formula":"mef","from":{"a":[],"x":[0]}}');
		TestTask.push('{"formula":"mef","from":{"a":[],"b":[1,-2,0,null]}}');
		TestTask.push('{"formula":"mef","from":{"a":[null]}}');
		TestTask.push('{"formula":"mef","from":{"a":[1],"p":[4,7]}}');
		TestTask.push('{"formula":"mef","from":{"a":[1,9],"p":[4,7]}}');
		}
	if(1){
		TestTask.push('{"compare": null}');
		TestTask.push('{"compare": []}');
		TestTask.push('{"compare": [1,2,3,4]}');
		TestTask.push('{"compare": ["a",2,"b"]}');
		TestTask.push('{"compare": ["n+1+x", "n*2", "(1+n*1)+(x-0)"]}');
		TestTask.push('{"compare": ["n+1+x", "==", "(1+n*1)+(x-0)"]}');
		}
	if(1){
		TestTask.push('{"optimize": null}');
		TestTask.push('{"optimize": "(c*2)-1", "from": null}');
		TestTask.push('{"optimize": "(c*2)-1", "from": {"c":null,"x":1}}');
		TestTask.push('{"optimize": "-1-(c*2)-10"}');
		}
	if(1){
		TestTask.push('{"equality": null}');
		TestTask.push('{"equality": []}');
		TestTask.push('{"equality": [1,2,3]}');
		TestTask.push('{"equality": ["",3]}');
		TestTask.push('{"equality": ["a","3*b"], "unknown":null}');
		TestTask.push('{"equality": ["a","3*b"], "unknown": ["a",null]}');
		TestTask.push('{"equality": ["a","3*b"], "unknown": ["b"]}');
		TestTask.push('{"equality": ["a","3*b"], "unknown": []}');
		}
	return TestTask;
};
...
var RunTestOfList = (TestTask)=>{
	TestTask.map((it)=>{
		var ResultValid = Valid(it);
		if(ResultValid===true){
			console.log(it,"OK");
			return;
			}
		console.log(ResultValid);
		console.log(ValidComment(it));
	});
};

//RunTestOfList(getTestsList());


module.exports = { Valid, ValidComment, getTestsList, RunTestOfList };
